...
/**
 *  String substitution for an alias to be added to build file
 */
function generateAliasReference(source, reference){
  return 'require.alias("' + source +'", "' + reference + '");\n';
}

/**
 *  Unintelligent joining of arguments to form a path like string (separated by slashes).
 *  Avoid gross string concats everywhere.
 */
function toPathLike(){
  ...
  this.json(function(err, conf){
        ...
        batch.push(function(done){
          ...
          self.lookup(dep, function(err, dir){
            ...
            builder.json(function(err, conf){
              ...
              var aliasBase = self.root ? self.conf.name : self.name;

              var topLevelAliases = [];
              var aliases = conf.scripts.map(function(script){
                var alias = toPathLike(aliasBase, 'deps', name, script);
                var source = toPathLike(builder.name, script);
                // A slight hack in that if we detect an index reference we will alias directly to it.
                if(script == "index.js")
                  topLevelAliases.push(generateAliasReference(toPathLike(builder.name, 'index.js'), toPathLike(name, 'index.js')));
                return generateAliasReference(source, alias);
              });

              // mixin the direct links with all script references
              aliases = aliases.concat(topLevelAliases);

              if (main) {
                // create a deps reference to handle nested code like root/lib/index.js
                var alias = toPathLike(aliasBase, 'deps', name, 'index.js');
                var source = toPathLike(builder.name, main);
                aliases.push(generateAliasReference(source, alias));

                // if a main is present then link directly to that for the component
                var alias = toPathLike(name, 'index.js');
                var source = toPathLike(builder.name, main);
                aliases.push(generateAliasReference(source, alias));
              }
    ...
    batch.end(function(err, res){
      ...
      if (self.root && conf.main) {
        var alias = toPathLike(name, 'index.js');
        var source = toPathLike(name, conf.main);
        res.push(generateAliasReference(source, alias));
      }

      // Convert all aliases to list
      fn(null, res.join(''));
    });
  });
};
